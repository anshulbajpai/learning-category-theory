...
object Reader {
  implicit def readMonad[R] = new Monad[({type λ[α] = Reader[R, α]})#λ] with Functor[({type λ[α] = Reader[R, α]})#λ] {
    override def flatMap[A, B](r: Reader[R, A], f: A => Reader[R, B]): Reader[R, B] = Reader(x => f(r.run(x)).run(x))

    override def map[A, B](r: Reader[R, A], f: A => B): Reader[R, B] = Reader(x => f(r.run(x)))
